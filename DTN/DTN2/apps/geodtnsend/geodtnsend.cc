...
main(int argc, char** argv)
{
   ...
   /* dtn_handle_t handle;
    ...
    geodtn_bundle_spec bundle_geospec;
  ...
  //  else err = dtn_open(&handle);
   ...
   // parse_eid(handle, &bundle_geospec.dest, arg_dest);
    ...
    // set the dtn options
    bundle_geospec.expiration = expiration;
    // loop, sending sends and getting replies.
    for (i = 0; i < copies; ++i)
    {


	/*	dtn::BundleRouter *router=dtn::BundleDaemon::GetInstance()->getrouter();
		dtn::GeoHistoryRouter *geoHistoryRouter=dynamic_cast<dtn::GeoHistoryRouter *>(router);
		if(geoHistoryRouter!=NULL)
		{
			geoHistoryRouter->handle_data_bundle(
					bundle_geospec.source,bundle_geospec.dest,bundle_geospec.expiration,
					data_source);
		}*/
