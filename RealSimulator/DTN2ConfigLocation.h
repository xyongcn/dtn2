...
class DTN2ConfigLocation {


private:
	static const string configPath;
	static DTN2ConfigLocation *instance;
	...
	DTN2ConfigLocation()
	{
		...
		fr.open(DTN2ConfigLocation::configPath.c_str(),ios::in);
	}
...
public:
    ...
    static DTN2ConfigLocation* GetInstance()
    {
        if(instance == NULL)  //判断是否第一次调用
        	instance = new DTN2ConfigLocation();
	    ...
	    while(!fr.eof() )//当curLine==null时，说明已到文件末，设置节点位置为配置文件里相关的最后一个位置。
	    {
			...
			ParseConfigFile *pcf=new ParseConfigFile(curLine);
				...
				//	cout<<curLine;
					...
					cout<<"DTN2getFirstConfigLocation"<<endl;
					cout<<"当前请求时间："<<time<<
							"  轨迹文件时间："<<pcf->gotTime
							<<"  时间间隔："<<this->sub<<endl;
...
const string DTN2ConfigLocation::configPath="./track.txt";
const string DTN2ConfigLocation::TAG="DTN2ConfigLocation";
DTN2ConfigLocation *DTN2ConfigLocation::instance=NULL;
