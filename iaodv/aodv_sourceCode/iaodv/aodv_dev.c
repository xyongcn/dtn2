...
int insert_net_dev(struct net_device *dev) {
	...
	if(g_mesh_dev==NULL) {
		...
		g_mesh_dev = new_dev;
		node->g_node_name = new_dev->ip;
//0913
		g_mesh_ip = new_dev->ip;
#ifdef DEBUG2
		printk("=================init g_mesh_dev%s=============\n",inet_ntoa(node->g_node_name));
#endif
	}
	//set the g_node_name as the smallest ip
	else if(node->g_node_name > new_dev->ip)
		node->g_node_name = new_dev->ip;
	/////**************************////
...
aodv_dev *get_netdev_by_ip(u_int32_t ip)
{
	aodv_dev *tmp_dev = net_dev_list;
