...
//caibingying-20150330
#ifdef RRediscovery
aodv_route *create_re_route(u_int32_t src_ip, u_int32_t dst_ip,
		unsigned char tos, u_int32_t dst_seq,struct net_device *dev,aodv_route *old_route) {
	aodv_route *tmp_entry;
#ifdef DEBUG2
	char src[20];
	char dst[20];
	...
	printk ("create_re_route: Creating %s to %s\n", src,dst);
#endif
	...
	tmp_entry->rediscovery = 1;

	//insert the r_route after the old one
	tmp_entry->next = old_route->next;
	if(old_route->next!=NULL)
		old_route->next->prev = tmp_entry;
	tmp_entry->prev = old_route;
	old_route->next = tmp_entry;

#ifdef DEBUG2
char nex[20];
strcpy(src, inet_ntoa(old_route->src_ip));
strcpy(dst, inet_ntoa(old_route->dst_ip));
strcpy(nex, inet_ntoa(old_route->next_hop));
printk ("OLD: %s to %s via %s with re:%d\n", src,dst,nex,old_route->rediscovery);
strcpy(src, inet_ntoa(old_route->next->src_ip));
strcpy(dst, inet_ntoa(old_route->next->dst_ip));
strcpy(nex, inet_ntoa(old_route->next->next_hop));
printk ("NEW: %s to %s via %s with re:%d\n", src,dst,nex,old_route->next->rediscovery);
#endif
...
int rreq_aodv_route(u_int32_t src_ip, u_int32_t dst_ip, unsigned char tos,
	...
	if(rd==1)
	{
		printk("It's a rediscovery rreq,don't return rrep anytime\n");
	}
#endif
...
//20141230 by cai		
		...
		if(rd==1)
		{
			...
			//if it's the first rrreq,do something as following,if not ,manage it like a normal rreq
			if(p!=NULL)
				tmp_route = p;
			else//this is the first rrreq,create a new route with rediscovery is 1
			{
#ifdef DEBUG2
				printk ("This is the first rrreq,create a new route with rediscovery is 1!\n");
#endif
				aodv_route *new_route = create_re_route(src_ip, dst_ip, tos, dst_seq,dev,tmp_route);
				...
				}//end if tmp_src_entry == NULL

				tmp_src_entry->num_routes++;

				/* Update values in the RT entry */
				aodv_dev *tmp_dev = get_netdev_by_name(dev->name); 
				new_route->dev = tmp_dev;
				new_route->path_metric = path_metric;
				new_route->dst_seq = dst_seq;	
				new_route->num_hops = hops;
				new_route->next_hop = next_hop->ip;
				route_write_lock(); //lifetime and route_valid are modified or read in packet_out.c - avoid conflicts
				new_route->lifetime = getcurrtime() + DELETE_PERIOD;
				new_route->state = ACTIVE;
				route_write_unlock();
#ifdef DEBUG2
				printk ("=====End in create re route,next_hop is %s!=====\n",inet_ntoa(new_route->next_hop));
#endif
				return 1;
			
			}//end if(tmp_route->rediscovery==0)
		}
#endif
...
aodv_route *find_aodv_route(u_int32_t src_ip, u_int32_t dst_ip,
	...
	if ((tmp_route == NULL) || (tmp_route->dst_ip != dst_ip)) { //not found!
		route_read_unlock();
			...
			printk("testWu: find_aodv_route return null1 \n");
		#endif
		return NULL;
	}
		...
		#ifdef WWTAO_DEBUG_GENRREP
			char wwtao_dst_ip[16];
			char wwtao_temp_dst_ip[16];
			strcpy (wwtao_dst_ip, inet_ntoa(dst_ip));
			strcpy (wwtao_temp_dst_ip, inet_ntoa(tmp_route->dst_ip));
			printk("testWu: tmp_route->dst_ip(%s) , dst_ip(%s) , temp_route->tos(%d) , tos(%d)  \n",wwtao_temp_dst_ip,wwtao_dst_ip,(int)tmp_route->tos,(int)tos);
		#endif

			if ((tmp_route->src_ip == src_ip) && (tmp_route->tos == tos)) {
				route_read_unlock();
		#ifdef WWTAO_DEBUG_GENRREP
			printk("testWu: find_aodv_route return route \n");
		#endif
				return tmp_route;
			}
			tmp_route = tmp_route->next;
	...
	route_read_unlock();
		#ifdef WWTAO_DEBUG_GENRREP
			printk("testWu: find_aodv_route return null2 \n");
		#endif
	return NULL;
...
aodv_route *find_rediscovery_aodv_route(u_int32_t src_ip, u_int32_t dst_ip,unsigned char tos) {
	...
	if ((tmp_route == NULL) || (tmp_route->dst_ip != dst_ip)) { //not found!
		...
		while ((tmp_route != NULL) && (tmp_route->dst_ip == dst_ip)) {
			if ((tmp_route->src_ip == src_ip) && (tmp_route->tos == tos) && (tmp_route->rediscovery==1)) {
				route_read_unlock();
				return tmp_route;
			}
			tmp_route = tmp_route->next;
